...
const TopbarInput = styled.div`
  display: flex;
  align-items: center;
  animation: ${TopbarInputAnim} ease-out 0.4s;
  position: relative;
...
export default function Search(props) {
  ...
  useEffect(() => {
    ...
    const toSearch = async () => {
      ...
      await fetch(
        `https://api.rawg.io/api/games?search=${game_title}&key=a4f2b0f2ba644c9db2ed4fbcc863365e`,
        {
          headers: {
  ...
  return (
    <StyledSearch>
      ...
      {loading ? (
        ...
        <NoResult>Sorry, we couldn't find what you were looking for.</NoResult>
      ) : null}
    </StyledSearch>
  );
}
